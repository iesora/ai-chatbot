(function(){
  ...
  function buildChatCard(container, opts){
    ...
    if(opts.onClose){
      ...
      closeBtn.type='button'; closeBtn.setAttribute('aria-label', '閉じる'); closeBtn.innerHTML='×';
      closeBtn.addEventListener('click', opts.onClose);
    }
    var body = h(card, 'div', 'hnaa-body');
    var msgs = h(body, 'div', 'hnaa-msgs');
    var form = h(card, 'form', 'hnaa-form');
    var input = h(form, 'textarea', 'hnaa-input'); input.placeholder = opts.placeholder || '質問を入力...'; input.rows = 2;
    var send = h(form, 'button', 'hnaa-send'); send.type='submit'; send.textContent='送信';

    var history = [];
    var pending = false;

    function addMsg(role, content){
      var row = h(msgs, 'div', 'hnaa-row '+(role==='user'?'is-user':'is-bot'));
    ...
    function setPending(v){
      pending = v;
      input.disabled = v; send.disabled = v;
    ...
    form.addEventListener('submit', async function(e){
      ...
      try{
        ...
        console.error(err);
        addMsg('assistant', 'エラーが発生しました: ' + (err && err.message ? err.message : String(err)));
      }finally{
        setPending(false);
      }
    });
    ...
    return { focus: function(){ try{ input.focus(); }catch(_){} } };
  }
  ...
  function renderModal(root, opts){
    ...
    if(String(opts.start_open||'').toLowerCase()==='true'){ open(); }
  }
  ...
  function render(root){
    ...
    if(!opts.api){ root.innerHTML = '<div class="hnaa-chatbot-error">APIが設定されていません。</div>'; return; }
